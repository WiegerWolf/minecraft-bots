 ...
 * 3. Find farm center if we don't have one (BEFORE tools - need to explore first!)
 * 4. Get tools if needed (only after we have a farm center or can craft locally)
...
export function createFarmingBehaviorTree(): BehaviorNode {
    return new Selector('Root', [
        ...
        // Priority 3: Find farm center if we don't have one (BEFORE tools!)
        new Sequence('EstablishFarm', [
            new Condition('NoFarmCenter', bb => !bb.farmCenter),
            new FindFarmCenter(),
        ]),

        // Priority 4: Get tools if needed - only after we have a farm center
        // Farmer requests logs from lumberjack via chat, does NOT chop trees itself
